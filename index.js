...
const jadwalnimek = require('./lib/add')
const get = require('got')
const { help } = require('./src/help')
const { wait, simih, getBuffer, h2k, generateMessageID, getGroupAdmins, getRandom, qrcode ,start, info, success, close } = require('./lib/functions')
...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'VERSION:3.0\n'
            + 'FN:Fian Senpai | Creator\n'
            + 'ORG: Creator ParadoX-Bot'
            + 'TEL;type=CELL;type=VOICE;waid=6285895811075:+6285895811075\n'
            + 'END:VCARD'
prefix = '/'
blocked = []
...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6285895811075@s.whatsapp.net"]
			const adminbotnumber = ["6285895811075@s.whatsapp.net"]
			...
			switch(command) {
				...
				case 'menu':
					...
					break
//case 'inpo':
//const informasi = `➽ *Nama Bot* : _ParadoX_\n➽ *prefix* : _${prefix}_\n➽ *Total Block* : _${blocked.length}_\n➽ *Aktif Sejak* : _26-02-2021_\n➽ *Instagram* : https://www.instagram.com/Fian_Skuy\n➽ *YouTube* : None\n➽ *Script By* : _FianSkuy_ [Mr.ParadoX]\n➽ *Note* : _Butuh Sc ? Pc Owner\n➽ *Sepecial Thangks To* :\n➽ _Allah SWT_ \n➽ _MahankBarBar_`
//client.sendMessage(informasi)
				case 'blocklist':
			...
			case 'owner':
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
				...
				case 'mlherolist':
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/mlherolist?apikey=`)
					icon = await getBuffer(anu.icon)
					client.sendMessage(from, icon, image, {quoted: mek})
					break
			    case 'randomhentai':
				    try {
						if (!isNsfw) return reply('❌ *Dosa Goblok* ❌')
				...
				case 'nsfwloli':
				    try {
						if (!isNsfw) return reply('❌ *Dasar Lolicon*❌')
			    ...
			    case 'nsfwneko':
				    try {
						if (!isNsfw) return reply('❌ *NekoPoi Liat sendir sono* ❌')
               ...
               case 'infogempa':
				...
				case 'tiktokstalk':
					try {
						...
						let { user, stats } = await tiktok.getUserProfileInfo(args[0])
						reply(mess.wait)
			...
			/*    case 'kudeta':
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ ParqdoX Broadcast]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
			...
			var awal = 0
			var akhir = 30
		for(awal + 1;awal <= akhir;awal++){reply('spammer '+'ke '+ awal)
				}
				break
				case 'simi':
					if (args.length < 1) return reply('Textnya mana um?')
				...
				//frendowner
				case `addadmin`:
                   if (!isOwner) return reply('Perintah ini hanya bisa di gunakan oleh Owner Bot', id)
		...
		/*	case 'randomanime':
            	 try {
						res = await fetchJson(`https://api.fdci.se/rep.php?gambar=cewe`, {method: 'get'})
						buffer = await getBuffer(res.result)
